...
import searchIcon from '../../Iconos/buscar.png';
...
const mockTickets = [
  ...
  // Ejemplo de shape (cuando agregues mocks reales):
  // { id: 1, title: 'No imprime', status: 'abierto', category: 'hardware', assignedTo: 'tec1', date: '2025-09-20', time: '10:00', usuarioId: 'u1', correo: 'ana@empresa.com' }
];

// ✅ Ahora acepta props opcionales: filtro (func) y titulo (string)
const TicketsModule = ({ filtro, titulo }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedFilter, setSelectedFilter] = useState('Todos');


  // ✅ Base de tickets: aplica filtro si llega por props
  const baseTickets = Array.isArray(mockTickets)
    ? (typeof filtro === 'function' ? mockTickets.filter(filtro) : mockTickets)
    : [];

  // Normalizador para comparar estatus
  const normalize = (s) => String(s || '').toLowerCase();

  const getTicketsByStatus = (status) => {
    if (status === 'Todos') return baseTickets;

    // Mapeo entre el texto del botón y el status interno del ticket
    // Botones: 'Abiertos' | 'En Proceso' | 'Terminados' | 'Cerrados'
    // Status ticket: 'abierto' | 'en-proceso' | 'terminado' | 'cerrado'
    const map = {
      'Abiertos': 'abierto',
    ...
    return baseTickets.filter(ticket => normalize(ticket.status) === normalize(target));
  };

  const getFilterCount = (filter) => {
    if (filter === 'Todos') return baseTickets.length;
    ...
    <div className="historial-container">
      <header className="tickets-header">
        <div className="header-info">
          {/* ✅ Muestra el titulo si viene por props; si no, deja el de siempre */}
          <h1>{titulo || 'Historial de Tickets'}</h1>
