...
CREATE OR REPLACE PROCEDURE USP_DELETE_DEPARTAMENTO (
...
BEGIN   
    ...
    FROM json_table(JSON_DEPARTAMENTO, '$[*]'
        COLUMNS (
            DEP_DEPARTAMENTO VARCHAR2(100) PATH '$.DEP_DEPARTAMENTO',
            DEP_NOMBRE VARCHAR2(100) PATH '$.DEP_NOMBRE',
            DEP_UBICACION VARCHAR2(100) PATH '$.DEP_UBICACION',
            DEP_ACTIVO CHAR(1) PATH '$.DEP_ACITIVO'
        )
    ) jq;
    ...
    FOR i IN V_TEMP_DEPTO.FIRST .. V_TEMP_DEPTO.LAST LOOP
        ...
        DELETE FROM HDK_DEPARTAMENTO
        WHERE DEP_DEPARTAMENTO = V_REGISTRO.DEP_DEPARTAMENTO;
    END LOOP;

    IF V_TEMP_DEPTO.COUNT = 1 THEN
        P_ID := V_REGISTRO.DEP_DEPARTAMENTO;
    ELSE
        SELECT MAX(DEP_DEPARTAMENTO) INTO P_ID FROM HDK_DEPARTAMENTO;
    END IF;
END;
